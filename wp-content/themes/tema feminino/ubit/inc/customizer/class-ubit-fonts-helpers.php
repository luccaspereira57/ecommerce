...
if ( ! class_exists( 'Ubit_Fonts_Helpers' ) ) :

	/**
	 * The Ubit Customizer class
	 */
	class Ubit_Fonts_Helpers {
		...
		public function __construct() {
			add_action( 'customize_controls_enqueue_scripts', array( $this, 'ubit_do_control_inline_scripts' ), 100 );
			add_action( 'wp_enqueue_scripts', array( $this, 'ubit_enqueue_google_fonts' ), 0 );
			add_action( 'enqueue_block_editor_assets', array( $this, 'ubit_enqueue_google_fonts' ), 0 );
			add_action( 'wp_ajax_ubit_get_all_google_fonts_ajax', array( $this, 'ubit_get_all_google_fonts_ajax' ) );
			add_filter( 'ubit_typography_customize_list', array( $this, 'ubit_add_to_font_customizer_list' ) );
		}
		/**
		 ...
		 */
		public function ubit_do_control_inline_scripts() {
			wp_localize_script(
				'ubit-typography-customizer',
				'ubit_customize',
				array(
					'nonce' => wp_create_nonce( 'ubit_customize_nonce' ),
				)
			);
			...
			wp_localize_script(
				'ubit-typography-customizer',
				'typography_defaults',
				$this->ubit_typography_default_fonts()
			);
		}
		 ...
		 */
		public static function ubit_typography_default_fonts() {
			$fonts = array(
				'inherit',
				'System Stack',
			...
			return apply_filters( 'ubit_typography_default_fonts', $fonts );
		}
		 ...
		 */
		public static function ubit_get_default_fonts( $filter = true ) {
			$ubit_font_defaults = array(
				// Body font.
				'body_font_family'             => 'Muli',
				'body_font_family_variants'    => 'regular,italic,600,600italic,700,700italic',
				'body_font_size'               => 16,
				...
				'menu_font_family_variants'    => 'regular,italic,500,500italic,600,600italic,700,700italic',
				'menu_font_weight'             => 700,
				'parent_menu_font_size'        => 14,
				'parent_menu_line_height'      => 90,
				'sub_menu_font_size'           => 14,
				'sub_menu_line_height'         => 24,
				'menu_font_category'           => 'sans-serif',
				'menu_font_transform'          => 'uppercase',

				// Heading font.
				'heading_font_family'          => 'Montserrat',
				'heading_font_family_variants' => 'regular,italic,500,500italic,600,600italic,700,700italic',
			...
			if ( $filter ) {
				return apply_filters( 'ubit_font_option_defaults', $ubit_font_defaults );
			}

			return $ubit_font_defaults;
		}

		/**
		 * Add Google Fonts to wp_head if needed.
		 */
		public function ubit_enqueue_google_fonts() {
			// Grab our options.
			$ubit_settings = wp_parse_args(
				get_option( 'ubit_setting', array() ),
				self::ubit_get_default_fonts()
			);

			// List our non-Google fonts.
			$not_google = str_replace( ' ', '+', self::ubit_typography_default_fonts() );
			...
			if ( ! empty( $font_settings ) ) {
				...
				foreach ( $font_settings as $key ) {
					...
					if ( ! isset( $ubit_settings[ $key ] ) ) {
						continue;
					}

					// If our value is still using the old format, fix it.
					if ( strpos( $ubit_settings[ $key ], ':' ) !== false ) {
						$ubit_settings[ $key ] = current( explode( ':', $ubit_settings[ $key ] ) );
					}

					// Replace the spaces in the names with a plus.
					$value = str_replace( ' ', '+', $ubit_settings[ $key ] );

					// Grab the variants using the plain name.
					$variants = $this->ubit_get_google_font_variants( $ubit_settings[ $key ], $key );
			...
			$google_fonts = apply_filters( 'ubit_typography_google_fonts', $google_fonts );

			// Get the subset.
			$subset = apply_filters( 'ubit_fonts_subset', '' );
		 ...
		 */
		public static function ubit_get_all_google_fonts( $amount = 'all' ) {
			...
			if ( apply_filters( 'ubit_alphabetize_google_fonts', true ) ) {
				asort( $fonts );
			}

			// Filter to allow us to modify the fonts array.
			return apply_filters( 'ubit_google_fonts_array', $fonts );
		}

		/**
		 * Return an array of all of our Google Fonts.
		 */
		public function ubit_get_all_google_fonts_ajax() {
			/*Bail if the nonce doesn't check out*/
			if ( ! isset( $_POST['ubit_customize_nonce'] ) || ! wp_verify_nonce( sanitize_key( $_POST['ubit_customize_nonce'] ), 'ubit_customize_nonce' ) ) {
				wp_die();
			}

			/*Do another nonce check*/
			check_ajax_referer( 'ubit_customize_nonce', 'ubit_customize_nonce' );
			...
			$fonts = $this->ubit_get_all_google_fonts();
		...
		public static function ubit_get_google_font_category( $font, $key = '' ) {
			...
			// Get our defaults.
			$defaults = self::ubit_get_default_fonts();
			...
			$fonts = self::ubit_get_all_google_fonts();
		 ...
		 * Example: ubit_get_google_font_variation( 'Open Sans' )
		 *
		 * @param      string $font   The font.
		 * @param      string $key    The key.
		 */
		public function ubit_get_google_font_variants( $font, $key = '' ) {
			...
			if ( $variants ) {
				...
				return implode( ',', apply_filters( 'ubit_typography_variants', $output ) );
			}
		}
		 ...
		 */
		public function ubit_add_to_font_customizer_list( $fonts ) {
			// Bail if we don't have our defaults.
			$ubit_settings = wp_parse_args(
				get_option( 'ubit_settings', array() ),
				self::ubit_get_default_fonts()
			);
			...
			$all_fonts    = $this->ubit_get_all_google_fonts();
			$select_fonts = $this->ubit_get_all_google_fonts( apply_filters( 'ubit_number_of_fonts', 200 ) );

			foreach ( $font_settings as $setting ) {
				// If we don't have a setting, keep going.
				if ( ! isset( $ubit_settings[ $setting ] ) ) {
					continue;
				}

				$id = strtolower( str_replace( ' ', '_', $ubit_settings[ $setting ] ) );

				if ( array_key_exists( $id, $select_fonts ) || in_array( $ubit_settings[ $setting ], self::ubit_typography_default_fonts() ) ) {
					continue;
				}

				$fonts[ strtolower( str_replace( ' ', '_', $ubit_settings[ $setting ] ) ) ] = array(
					'name'     => $ubit_settings[ $setting ],
		 ...
		 */
		public static function ubit_get_font_family_css( $font, $settings, $default ) {
			$ubit_settings = wp_parse_args(
			...
			$no_quotes = array(
				...
				'Trebuchet MS, Helvetica, sans-serif',
				'Verdana, Geneva, sans-serif',
				apply_filters( 'ubit_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' ),
			);

			/*Get our font*/
			$font_family = $ubit_settings[ $font ];

			if ( 'System Stack' == $font_family ) {
				$font_family = apply_filters( 'ubit_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' );
			}
			...
			if ( in_array( $font_family, $no_quotes ) ) {
				...
				$wrapper_start = '"';
				$wrapper_end   = '"' . self::ubit_get_google_font_category( $font_family, $font );
			}
...
Ubit_Fonts_Helpers::get_instance();
