...
wp.customize.controlConstructor['ubit-range-slider'] = wp.customize.Control.extend({
	...
	ready: function() {
		...
		var control = this,
			...
			inputDefault,
			changeAction,
			controlClass = '.customize-control-ubit-range-slider',
			footerActions = jQuery( '#customize-footer-actions' );

		// Set up the sliders.
		jQuery( '.ubit-slider' ).each( function() {
		...
		jQuery( controlClass + ' .ubit_range_value input[type=number]' ).on( 'input', function() {
			...
			jQuery( this ).closest( 'label' ).find( '.ubit-slider' ).slider( 'value', parseFloat( value ) ).change();
		});

		// Handle the reset button.
		jQuery( controlClass + ' .ubit-reset' ).on( 'click', function() {
			var icon         = jQuery( this ),
				visible_area = icon.closest( '.ubit-range-title-area' ).next( '.ubit-range-slider-areas' ).children( 'label:visible' ),
				input        = visible_area.find( 'input[type=number]' ),
				slider_value = visible_area.find( '.ubit-slider' ),
				visual_value = visible_area.find( '.ubit_range_value' ),
		...
		jQuery( controlClass + ' .ubit-range-slider-control' ).each( function() {
			var _this = jQuery( this );
			_this.find( '.ubit-device-controls' ).children( 'span:first-child' ).addClass( 'selected' );
			_this.find( '.range-option-area:first-child' ).show();
		});

		// Do stuff when device icons are clicked.
		jQuery( controlClass + ' .ubit-device-controls > span' ).on( 'click', function( event ) {
			var device = jQuery( this ).data( 'option' );

			jQuery( controlClass + ' .ubit-device-controls span' ).each( function() {
				var _this = jQuery( this );
			...
			jQuery( controlClass + ' .ubit-range-slider-areas label' ).each( function() {
				var _this = jQuery( this );
				if ( device == _this.attr( 'data-option' ) ) {
					_this.show();
					_this.siblings().hide();
