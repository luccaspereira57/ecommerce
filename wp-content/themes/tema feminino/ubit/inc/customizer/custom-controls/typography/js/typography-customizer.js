( function( api ) {
	...
	api.controlConstructor['ubit-customizer-typography'] = api.Control.extend( {
		ready: function() {
			var control = this;

			control.container.on( 'change', '.ubit-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the ubit_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'ubit_get_all_google_fonts_ajax',
								ubit_customize_nonce: ubit_customize.nonce
							},
							async: false,
							dataType: 'json',
						});
						...
						if ( id in fonts ) {
							...
							jQuery( '.ubit-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.ubit-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.ubit-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.ubit-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.ubit-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.ubit-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.ubit-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.ubit-font-variant' );
			...
			control.container.on( 'change', '.ubit-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.ubit-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.ubit-font-transform select',
				function() {
...
jQuery( document ).ready( function($) {
	...
	jQuery( '.ubit-font-family select' ).selectWoo();
	jQuery( '.ubit-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".ubit-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.ubit-font-variant' ).hide();
		}
	});
