...
class Merlin {
	...
	public function __construct( $config = array(), $strings = array() ) {
		...
		require_once MERLIN_DIR . 'includes/class-merlin-logger.php';
		$this->logger = Merlin_Logger::get_instance();

		// Get TGMPA
		require_once MERLIN_DIR . 'includes/tgm-plugin-activation.php';
		if ( class_exists( 'TGM_Plugin_Activation' ) ) {
			$this->tgmpa = isset( $GLOBALS['tgmpa'] ) ? $GLOBALS['tgmpa'] : TGM_Plugin_Activation::get_instance();

			// Hide TGMPA notices until setup wizard has started at least once
			if ( ! get_option( 'merlin_' . $this->slug . '_started' ) ) {
				$this->tgmpa->has_notices = false;
			}
			else {
				$this->tgmpa->has_notices = true;
			}
		}

		add_action( 'admin_init', array( $this, 'merlin_disable_default_elementor_color_font' ) );
		add_action( 'admin_init', array( $this, 'merlin_required_classes' ) );
		add_action( 'admin_init', array( $this, 'merlin_redirect' ), 30 );
		add_action( 'admin_init', array( $this, 'merlin_steps' ), 30, 0 );
		add_action( 'admin_menu', array( $this, 'merlin_add_admin_menu' ) );
		add_action( 'admin_init', array( $this, 'merlin_admin_page' ), 30, 0 );
		add_action( 'admin_init', array( $this, 'merlin_ignore' ), 5 );
		add_action( 'admin_footer', array( $this, 'merlin_svg_sprite' ) );
		add_filter( 'tgmpa_load', array( $this, 'merlin_load_tgmpa' ), 10, 1 );
		add_action( 'wp_ajax_merlin_content', array( $this, 'merlin_ajax_content' ), 10, 0 );
		add_action( 'wp_ajax_merlin_get_total_content_import_items', array( $this, 'merlin_ajax_get_total_content_import_items' ), 10, 0 );
		add_action( 'wp_ajax_merlin_plugins', array( $this, 'merlin_ajax_plugins' ), 10, 0 );
		add_action( 'wp_ajax_merlin_child_theme', array( $this, 'merlin_generate_child' ), 10, 0 );
		add_action( 'wp_ajax_merlin_activate_license', array( $this, 'merlin_ajax_activate_license' ), 10, 0 );
		add_action( 'wp_ajax_merlin_update_selected_import_data_info', array( $this, 'merlin_update_selected_import_data_info' ), 10, 0 );
		add_action( 'wp_ajax_merlin_import_finished', array( $this, 'merlin_import_finished' ), 10, 0 );
		add_filter( 'pt-importer/new_ajax_request_response_data', array( $this, 'merlin_pt_importer_new_ajax_request_response_data' ) );
		add_action( 'import_start', array( $this, 'merlin_before_content_import_setup' ) );
		add_action( 'merlin_after_all_import', array( $this, 'merlin_after_content_import_setup' ) );
		add_action( 'admin_init', array( $this, 'merlin_register_import_files' ) );
		add_action(
			'admin_init',
			function () {
	 ...
	 * @return void
	 */
	public function merlin_disable_default_elementor_color_font() {
	 ...
	 */
	public function merlin_required_classes() {
		if ( ! class_exists( '\WP_Importer' ) ) {
			require ABSPATH . '/wp-admin/includes/class-wp-importer.php';
		}

		require_once MERLIN_DIR . 'includes/class-merlin-downloader.php';

		$this->importer = new ProteusThemes\WPContentImporter2\Importer( array( 'fetch_attachments' => true ), $this->logger );

		require_once MERLIN_DIR . 'includes/class-merlin-widget-importer.php';

		if ( ! class_exists( 'WP_Customize_Setting' ) ) {
			require_once ABSPATH . 'wp-includes/class-wp-customize-setting.php';
		}

		require_once MERLIN_DIR . 'includes/class-merlin-customizer-option.php';
		require_once MERLIN_DIR . 'includes/class-merlin-customizer-importer.php';
		require_once MERLIN_DIR . 'includes/class-merlin-redux-importer.php';
		require_once MERLIN_DIR . 'includes/class-merlin-hooks.php';
	 ...
	 */
	public function merlin_redirect() {
	 ...
	 */
	public function merlin_ignore() {

		// Bail out if not on correct page.
		if ( ! isset( $_GET['_wpnonce'] ) || ( ! wp_verify_nonce( $_GET['_wpnonce'], 'merlin-ignore-nonce' ) || ! is_admin() || ! isset( $_GET[ $this->ignore ] ) || ! current_user_can( 'manage_options' ) ) ) {
			return;
		}
	 ...
	 */
	public function merlin_load_tgmpa( $status ) {
	 ...
	 */
	public function merlin_add_admin_menu() {
	 ...
	 * Add the admin page.
	 */
	public function merlin_admin_page() {
		...
		// Enqueue styles.
		wp_enqueue_style( 'merlin-admin-styles', MERLIN_URI . 'assets/css/merlin.css', array( 'wp-admin' ), '1.0.0' );

		// Enqueue javascript.
		wp_enqueue_script( 'merlin-admin-scripts', MERLIN_URI . 'assets/js/merlin.js', array( 'jquery-core' ), '1.0.0', true );

		$texts = array(
			'something_went_wrong' => esc_html__( 'Something went wrong. Please refresh the page and try again!', 'ubit' ),
		);
		...
		<div class="merlin__wrapper">
			...
			<?php
			// Set as already-run the setup wizard
			// Currently disabled for better UX
			//$ignore_url = wp_nonce_url( admin_url( '?' . $this->ignore . '=true' ), 'merlin-ignore-nonce' );
	 ...
	 * SVG
	 */
	public function merlin_svg_sprite() {

		// Define SVG sprite file.
		$svg = MERLIN_DIR . 'assets/images/sprite.svg';
	...
	public function svg( $args = array() ) {
		...
		if ( empty( $args ) ) {
			return esc_html__( 'Please define default parameters in the form of an array.', 'ubit' );
		}

		// Define an icon.
		if ( false === array_key_exists( 'icon', $args ) ) {
			return esc_html__( 'Please define an SVG icon filename.', 'ubit' );
		}
	 ...
	 * Setup steps.
	 */
	public function merlin_steps() {

		$this->steps = array(
			'welcome' => array(
				'name'    => esc_html__( 'Welcome', 'ubit' ),
		...
		if ( ! empty( $this->import_files ) ) {
			$this->steps['content'] = array(
				'name' => esc_html__( 'Content', 'ubit' ),
				'view' => array( $this, 'content' ),
			);
		}

		$this->steps['ready'] = array(
			'name' => esc_html__( 'Ready', 'ubit' ),
	...
	protected function welcome() {
		...
		update_option( 'merlin_' . $this->slug . '_started', time() );
		?>
		...
		<?php
		$this->logger->debug( esc_html__( 'The welcome step has been displayed', 'ubit' ) );
	}
	...
	protected function license() {
		...
		<?php
		$this->logger->debug( esc_html__( 'The license activation step has been displayed', 'ubit' ) );
	}
	...
	protected function child() {
		...
		<?php
		$this->logger->debug( esc_html__( 'The child theme installation step has been displayed', 'ubit' ) );
	}
	...
	protected function plugins() {
						...
						<?php foreach ( $required_plugins as $slug => $plugin ) : ?>
							<li data-slug="<?php echo esc_attr( $slug ); ?>">
								...
								<label for="default_plugins_<?php echo esc_attr( $slug ); ?>">
									...
									<span class="badge">
									<span class="hint--top" aria-label="<?php esc_html_e( 'Required', 'ubit' ); ?>">
										<?php esc_html_e( 'req', 'ubit' ); ?>
									</span>
								</span>
								</label>
							</li>
		...
		<?php
		$this->logger->debug( esc_html__( 'The plugin installation step has been displayed', 'ubit' ) );
	}
	...
	protected function content() {
		...
		<div class="merlin__content--transition">
			...
			<p><?php echo esc_html( $paragraph ); ?></p>
			<div>
				<p><?php esc_html_e( 'Make sure you reset the site to default installation values before importing demo data.', 'ubit' ); ?></p>
				<?php
				$link = sprintf(
					wp_kses(
						/* translators: suggested plugin link */
						__( 'You can use the <a href="%s" target="_blank" rel="noopener noreferrer">WP Reset</a> plugin to reset and run through demos.', 'ubit' ),
						array(
							'a' => array(
								'href' => array(),
								'target' => array(),
			...
			<?php if ( 1 < count( $this->import_files ) ) : ?>
				<p><?php esc_html_e( 'Select which demo data you want to import:', 'ubit' ); ?></p>
			<?php endif; ?>
			<div class="merlin__demos">
					...
					<div class="merlin__demo">
						<div class="merlin__demo-image">
							<?php if ( ! empty( $demo_image ) ) : ?>
								<img src="<?php echo esc_url( $demo_image ); ?>" alt="<?php echo esc_attr( 'Demo Image', 'ubit' ); ?>">
							<?php endif; ?>
						</div>
						...
						<div class="merlin__demo-content">
							...
							<div class="merlin__demo-cta">
								<a class="merlin__demo-button" href="<?php echo esc_url( $demo_preview_url ); ?>" target="_blank"><?php esc_html_e( 'Preview', 'ubit' ); ?></a>
								<button data-content="<?php echo esc_attr( $index ); ?>" class="merlin__demo-button js-select-demo"><?php esc_html_e( 'Select', 'ubit' ); ?></button>
							</div>
						</div>
					</div>
		...
		<?php
		$this->logger->debug( esc_html__( 'The content import step has been displayed', 'ubit' ) );
	}
	...
	protected function ready() {
		...
		<?php
		$this->logger->debug( esc_html__( 'The final step has been displayed', 'ubit' ) );
	}
	 ...
	 */
	public function merlin_generate_child() {
		...
		if ( ! file_exists( $path ) ) {
			...
			$wp_filesystem->put_contents( $path . '/style.css', $this->generate_child_style_css( $this->theme->template, $this->theme->name, $this->theme->author, $this->theme->version, $this->theme ) );
			...
			$this->logger->debug( esc_html__( 'The existing child theme was activated', 'ubit' ) );
		...
		$this->logger->debug( esc_html__( 'The newly generated child theme was activated', 'ubit' ) );
	 ...
	 */
	public function merlin_ajax_activate_license() {

		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Yikes! The theme activation failed. Please try again or contact support.', 'ubit' ),
				)
			);
		}
		...
		if ( empty( $_POST['license_key'] ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Please add your license key before attempting to activate one.', 'ubit' ),
				)
			);
		}
		...
		$this->logger->debug( esc_html__( 'The license activation was performed with the following results', 'ubit' ), $result );
	...
	protected function edd_activate_license( $license ) {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = esc_html__( 'An error occurred, please try again.', 'ubit' );
			}
		} else {
				...
				switch ( $license_data->error ) {
					...
					case 'expired':
						$message = sprintf(
							...
							esc_html__( 'Your license key expired on %s.', 'ubit' ),
							date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) )
						);
						break;

					case 'revoked':
						$message = esc_html__( 'Your license key has been disabled.', 'ubit' );
						break;

					case 'missing':
						$message = esc_html__( 'This appears to be an invalid license key. Please try again or contact support.', 'ubit' );
						break;

					case 'invalid':
					case 'site_inactive':
						$message = esc_html__( 'Your license is not active for this URL.', 'ubit' );
						break;

					case 'item_name_mismatch':
						/* translators: EDD Item Name */
						$message = sprintf( esc_html__( 'This appears to be an invalid license key for %s.', 'ubit' ), $this->edd_item_name );
						break;

					case 'no_activations_left':
						$message = esc_html__( 'Your license key has reached its activation limit.', 'ubit' );
						break;
	...
	public function generate_child_functions_php( $slug ) {
		...
		$output = "
			...
			/**
			 * Load the child theme style.css file.
			 *
			 * @link https://developer.wordpress.org/themes/advanced-topics/child-themes/
			 */
			function {$slug_no_hyphens}_child_enqueue_style() {
			    wp_enqueue_style(
			        '{$slug}-child-style',
			...
			add_action( 'wp_enqueue_scripts', '{$slug_no_hyphens}_child_enqueue_style', 30 );
		...
		$this->logger->debug( esc_html__( 'The child theme functions.php content was generated', 'ubit' ) );
	 ...
	 */
	public function generate_child_style_css( $slug, $parent, $author, $version, $theme ) {
		$tags = is_array( $theme->get('Tags') ) ? implode( ', ', $theme->get('Tags') ) : $theme->get('Tags');

		$output = "
			/*
			Theme Name: {$parent} Child
			Theme URI: {$theme->get('ThemeURI')}
			Description: This is a child theme of {$parent} theme. {$theme->get('Description')}
			...
			Domain Path: {$theme->get('DomainPath')}
			Tags: {$tags}
			*/
		...
		$this->logger->debug( esc_html__( 'The child theme style.css content was generated', 'ubit' ) );

		return apply_filters( 'merlin_generate_child_style_css', $output, $slug, $parent, $author, $version, $theme );
	}
	...
	public function generate_child_screenshot( $path ) {
		...
		if ( ! empty( $screenshot ) && file_exists( $screenshot ) ) {
			...
			$this->logger->debug( esc_html__( 'The child theme screenshot was copied to the child theme, with the following result', 'ubit' ), array( 'copied' => $copied ) );
		} else {
			$this->logger->debug( esc_html__( 'The child theme screenshot was not generated, because of these results', 'ubit' ), array( 'screenshot' => $screenshot ) );
		}
	}
	 ...
	 */
	public function merlin_ajax_plugins() {
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Activating', 'ubit' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Updating', 'ubit' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Installing', 'ubit' ),
				);
				break;
			}
		}
		...
		if ( $json ) {
			$this->logger->debug(
				esc_html__( 'A plugin with the following data will be processed', 'ubit' ),
				array(
			...
			$json['message'] = esc_html__( 'Installing', 'ubit' );
			wp_send_json( $json );
		} else {
			$this->logger->debug(
				esc_html__( 'A plugin with the following data was processed', 'ubit' ),
				array(
					'plugin_slug' => $_POST['slug'],
				)
			);
			...
			wp_send_json(
				array(
					'done'    => 1,
					'message' => esc_html__( 'Success', 'ubit' ),
				)
			);
		}
	 ...
	 */
	public function merlin_ajax_content() {
		static $content = null;
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
			$this->logger->error( esc_html__( 'The content importer AJAX call failed to start, because of incorrect data', 'ubit' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid content!', 'ubit' ),
				)
			);
		}
		...
		if ( isset( $_POST['proceed'] ) ) {
			if ( is_callable( $this_content['install_callback'] ) ) {
				$this->logger->info(
					esc_html__( 'The content import AJAX call will be executed with this import data', 'ubit' ),
					array(
		...
		if ( $json ) {
			...
			$this->logger->error(
				esc_html__( 'The content import AJAX call failed with this passed data', 'ubit' ),
				array(
	 ...
	 */
	public function merlin_ajax_get_total_content_import_items() {
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) && empty( $_POST['selected_index'] ) ) {
			$this->logger->error( esc_html__( 'The content importer AJAX call for retrieving total content import items failed to start, because of incorrect data.', 'ubit' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid data!', 'ubit' ),
				)
			);
		}
	...
	public function get_import_data_info( $selected_import_index = 0 ) {
		...
		if (
			! empty( $this->import_files[ $selected_import_index ]['import_ss3_slider_file_url'] ) ||
			! empty( $this->import_files[ $selected_import_index ]['local_import_ss3_slider_file'] )
		) {
			$import_data['smart_slider_3'] = true;
		}
	...
	protected function get_import_data( $selected_import_index = 0 ) {
		...
		if ( ! empty( $import_files['content'] ) ) {
			$content['content'] = array(
				'title'            => esc_html__( 'Content', 'ubit' ),
				'description'      => esc_html__( 'Demo content data.', 'ubit' ),
				'pending'          => esc_html__( 'Pending', 'ubit' ),
				'installing'       => esc_html__( 'Installing', 'ubit' ),
				'success'          => esc_html__( 'Success', 'ubit' ),
		...
		if ( ! empty( $import_files['widgets'] ) ) {
			$content['widgets'] = array(
				'title'            => esc_html__( 'Widgets', 'ubit' ),
				'description'      => esc_html__( 'Sample widgets data.', 'ubit' ),
				'pending'          => esc_html__( 'Pending', 'ubit' ),
				'installing'       => esc_html__( 'Installing', 'ubit' ),
				'success'          => esc_html__( 'Success', 'ubit' ),
		...
		if ( ! empty( $import_files['sliders'] ) ) {
			$content['sliders'] = array(
				'title'            => esc_html__( 'Revolution Slider', 'ubit' ),
				'description'      => esc_html__( 'Sample Revolution sliders data.', 'ubit' ),
				'pending'          => esc_html__( 'Pending', 'ubit' ),
				'installing'       => esc_html__( 'Installing', 'ubit' ),
				'success'          => esc_html__( 'Success', 'ubit' ),
		...
		if ( ! empty( $import_files['smart_slider_3'] ) ) {
			$content['smart_slider_3'] = array(
				'title'            => esc_html__( 'Smart Slider 3', 'ubit' ),
				'description'      => esc_html__( 'Sample Smart Slider 3 sliders data.', 'ubit' ),
				'pending'          => esc_html__( 'Pending', 'ubit' ),
				'installing'       => esc_html__( 'Installing', 'ubit' ),
				'success'          => esc_html__( 'Success', 'ubit' ),
				'install_callback' => array( $this, 'import_ss3_sliders' ),
				'checked'          => $this->is_possible_upgrade() ? 0 : 1,
				'data'             => $import_files['smart_slider_3'],
			);
		}

		if ( ! empty( $import_files['options'] ) ) {
			$content['options'] = array(
				'title'            => esc_html__( 'Options', 'ubit' ),
				'description'      => esc_html__( 'Sample theme options data.', 'ubit' ),
				'pending'          => esc_html__( 'Pending', 'ubit' ),
				'installing'       => esc_html__( 'Installing', 'ubit' ),
				'success'          => esc_html__( 'Success', 'ubit' ),
		...
		if ( ! empty( $import_files['redux'] ) ) {
			$content['redux'] = array(
				'title'            => esc_html__( 'Redux Options', 'ubit' ),
				'description'      => esc_html__( 'Redux framework options.', 'ubit' ),
				'pending'          => esc_html__( 'Pending', 'ubit' ),
				'installing'       => esc_html__( 'Installing', 'ubit' ),
				'success'          => esc_html__( 'Success', 'ubit' ),
		...
		if ( false !== has_action( 'merlin_after_all_import' ) ) {
			$content['after_import'] = array(
				'title'            => esc_html__( 'After import setup', 'ubit' ),
				'description'      => esc_html__( 'After import setup.', 'ubit' ),
				'pending'          => esc_html__( 'Pending', 'ubit' ),
				'installing'       => esc_html__( 'Installing', 'ubit' ),
				'success'          => esc_html__( 'Success', 'ubit' ),
	...
	public function import_revolution_sliders( $file ) {
		...
		$this->logger->info( esc_html__( 'The revolution slider import was executed', 'ubit' ) );
	...
	/**
	 * Import Smart Slider 3 slider.
	 *
	 * @param string $file Path to the Smart Slider 3 slider ss3 file.
	 */
	public function import_ss3_sliders( $files ) {
		...
		if ( is_array( $files ) ) {
			foreach ( $files as $file ) {
				$response = SmartSlider3::import( $file );
			}
		}
		else {
			$response = SmartSlider3::import( $files );
		}

		$this->logger->info( esc_html__( 'The Smart Slider 3 slider import was executed', 'ubit' ) );
	 ...
	 */
	public function merlin_pt_importer_new_ajax_request_response_data( $data ) {
		$data['url']      = admin_url( 'admin-ajax.php' );
		$data['message']  = esc_html__( 'Installing', 'ubit' );
		$data['proceed']  = 'true';
	 ...
	 */
	public function merlin_after_content_import_setup( $selected_import_index ) {
		$selected_demo = $this->import_files[ $selected_import_index ];

		// Set static homepage.
		$homepage = get_page_by_title( apply_filters( 'merlin_content_home_page_title', $selected_demo['homepage'] ) );

		if ( $homepage ) {
			update_option( 'page_on_front', $homepage->ID );
			update_option( 'show_on_front', 'page' );

			$this->logger->debug( esc_html__( 'The home page was set', 'ubit' ), array( 'homepage_id' => $homepage ) );
		}

		// Set static blog page.
		$blogpage = get_page_by_title( apply_filters( 'merlin_content_blog_page_title', $selected_demo['blog_page'] ) );

		if ( $blogpage ) {
			update_option( 'page_for_posts', $blogpage->ID );
			update_option( 'show_on_front', 'page' );

			$this->logger->debug( esc_html__( 'The blog page was set', 'ubit' ), array( 'blog_page_id' => $blogpage ) );
		}
		...
		// Get menus
		$locations['topbar']     = $this->merlin_get_menu_by_name( $selected_demo['topbar_menu'] );
		$locations['primary']    = $this->merlin_get_menu_by_name( $selected_demo['primary_menu'] );
		$locations['footer']     = $this->merlin_get_menu_by_name( $selected_demo['footer_menu'] );
	 ...
	 */
	public function merlin_get_menu_by_name( $name ) {
	 ...
	 */
	public function merlin_before_content_import_setup() {
		...
		if ( ! empty( $hello_world ) ) {
			...
			$this->logger->debug( esc_html__( 'The Hello world post status was set to draft', 'ubit' ) );
		}
	}
	 ...
	 */
	public function merlin_register_import_files() {
	...
	public function validate_import_file_info( $import_files ) {
		...
		foreach ( $import_files as $import_file ) {
			if ( ! empty( $import_file['import_file_name'] ) ) {
				...
				$this->logger->warning( esc_html__( 'This predefined demo import does not have the name parameter: import_file_name', 'ubit' ), $import_file );
			}
		}
	...
	public function get_import_files_paths( $selected_import_index ) {
		...
		$import_files   = array(
			...
			'sliders' => '',
			'smart_slider_3' => '',
			// 'plugins' => '', // TODO: obsolete
		);

		$downloader = new Merlin_Downloader();

		// $import_files['plugins'] = $selected_import_data['plugins']; // TODO: obsolete
		...
		// Get Smart Slider 3 slider import file as well. If defined!
		if ( ! empty( $selected_import_data['import_ss3_slider_file_url'] ) ) {
			// Setup filename path to save the customizer content.
			$ss3_slider_filename = 'slider-' . $base_file_name . '.ss3';

			// Retrieve the content import file.
			$import_files['smart_slider_3'] = $downloader->fetch_existing_file( $ss3_slider_filename );

			// Download the file, if it's missing.
			if ( empty( $import_files['smart_slider_3'] ) ) {
				$import_files['smart_slider_3'] = $downloader->download_file( $selected_import_data['import_ss3_slider_file_url'], $ss3_slider_filename );
			}

			// Reset the variable, if there was an error.
			if ( is_wp_error( $import_files['smart_slider_3'] ) ) {
				$import_files['smart_slider_3'] = '';
			}
		} elseif ( ! empty( $selected_import_data['local_import_ss3_slider_file'] ) ) {
			if ( is_array( $selected_import_data['local_import_ss3_slider_file'] ) ) {
				$import_files['smart_slider_3'] = array();
				foreach ( $selected_import_data['local_import_ss3_slider_file'] as $index => $slider_item ) {
					if ( file_exists( $slider_item ) ) {
						$import_files['smart_slider_3'][] = $slider_item;
					}
				}
			}
			else {
				if ( file_exists( $selected_import_data['local_import_ss3_slider_file'] ) ) {
					$import_files['smart_slider_3'] = $selected_import_data['local_import_ss3_slider_file'];
				}
			}
		}
	 ...
	 */
	public function merlin_update_selected_import_data_info() {
		...
		<?php foreach ( $import_info as $slug => $available ) : ?>
			...
			<li class="merlin__drawer--import-content__list-item status status--Pending" data-content="<?php echo esc_attr( $slug ); ?>">
				...
				<label for="default_content_<?php echo esc_attr( $slug ); ?>">
					<i></i><span><?php echo esc_html( ucwords( str_replace( '_', ' ', $slug ) ) ); ?></span>
				</label>
			</li>
	 ...
	 */
	public function merlin_import_finished() {
